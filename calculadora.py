...
with st.sidebar:
    ...
    # AQUI ESTÁ A CORREÇÃO: "value=None" garante que a tela abra vazia, 
    # e "min_value" permite retroagir a data com segurança.
...
if not data_inicial or not data_final:
    ...
    # Se as datas estão corretas, libera a calculadora completa!
    ...
    df_lancamentos = st.data_editor(
        ...
        hide_index=True,
        column_config={
            "Data": st.column_config.DateColumn("Data", min_value=date(1980, 1, 1), format="DD/MM/YYYY"),
    ...
    if btn_processar:
        ...
        if tipo_juros == "Compostos":
            taxa_periodo = (1 + taxa_mensal_dec) ** (Decimal(dias_totais) / Decimal(30)) - 1
            valor_juros = saldo_atual * taxa_periodo
        else: 
            taxa_periodo = taxa_mensal_dec * (Decimal(dias_totais) / Decimal(30))
            valor_juros = saldo_atual * taxa_periodo
        ...
        col4.metric("Período (Dias)", dias_totais)
        ...
        df_resumo_export = pd.DataFrame({
            ...
            "Valor": [float(saldo_atual), float(valor_juros), float(saldo_final_absoluto), dias_totais]
        })
        
        dados_resumo_pdf = {"Original": float(saldo_atual), "Juros": float(valor_juros), "Final": float(saldo_final_absoluto), "Dias": dias_totais}
